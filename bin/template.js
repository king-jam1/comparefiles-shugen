...
(function (mod) {
  ...
  function DiffView(mv, type) {
    ...
    // 作用样式
    this.classes = {
      chunk: `CodeMirror-merge-${type}-chunk`,
      start: `CodeMirror-merge-${type}-chunk-start`,
      end: `CodeMirror-merge-${type}-chunk-end`,
      insert: `CodeMirror-merge-${type}-inserted`,
      del: `CodeMirror-merge-${type}-deleted`,
      connect: `CodeMirror-merge-${type}-connect`,
    };
  ...
  DiffView.prototype = {
    ...
    init: function (pane, orig, options) {
      ...
      // Liao: 添加鼠标监听
      this.edit.on("cursorActivity", (cm) => {
        if (this.lastEditClickLines) {
          // 恢复默认底色（class）
          this.lastEditClickLines.forEach((linePos) => {
            if (linePos.editFrom === linePos.editTo) {
              cm.removeLineClass(
                linePos.editFrom,
                "background",
                "CodeMirror-merge-lighlight-click-chunk-start"
              );
            } else {
              for (let i = linePos.editFrom; i < linePos.editTo; i++) {
                cm.addLineClass(i, "background", `${this.classes.chunk}`);
                cm.addLineClass(i, "background", `${this.classes.start}`);
                cm.addLineClass(i, "background", `${this.classes.end}`);
              }
            }
          });
        }
        if (this.lastOrigClickLines) {
          // 恢复默认底色（class）
          this.lastOrigClickLines.forEach((linePos) => {
            if (linePos.origFrom === linePos.origTo) {
              this.orig.removeLineClass(
                linePos.origFrom,
                "background",
                "CodeMirror-merge-lighlight-click-chunk-start"
              );
            } else {
              for (let i = linePos.origFrom; i < linePos.origTo; i++) {
                this.orig.addLineClass(
                  i,
                  "background",
                  `${this.classes.chunk}`
                );
                ...
                this.orig.addLineClass(i, "background", `${this.classes.end}`);
              }
            }
          });
        }
        var cursorLine = cm.getCursor().line;
        this.highLightChunks = getChunks(this.diff).filter(
          (hightLightChunk) =>
            hightLightChunk.editFrom <= cursorLine &&
            cursorLine < hightLightChunk.editTo
        );
        this.lastEditClickLines = []; // 记录下这次的行，下次开始前恢复样式用
        this.lastOrigClickLines = [];
        const that = this;
        this.highLightChunks.forEach((hightLightChunk) => {
          const { origFrom, origTo, editFrom, editTo } = hightLightChunk;
          if (origFrom === origTo) {
            that.lastOrigClickLines.push({ origFrom, origTo });
            that.orig.removeLineClass(
              origFrom,
              "background",
              `${that.classes.chunk}`
            );
          ...
          if (editFrom === editTo) {
            that.lastEditClickLines.push({ editFrom, editTo });
            cm.removeLineClass(editFrom, "background", `${that.classes.chunk}`);
            cm.removeLineClass(editFrom, "background", `${that.classes.start}`);
            cm.removeLineClass(editFrom, "background", `${that.classes.end}`);
            cm.removeLineClass(
              editFrom,
              "wrap",
              "CodeMirror-merge-collapsed-line"
            ...
            for (let i = origFrom; i < origTo; i++) {
              ...
              that.orig.removeLineClass(i, "background", `${that.classes.end}`);
              that.orig.addLineClass(
                i,
                "background",
                "CodeMirror-merge-lighlight-click-chunk"
              );
            }
            // edit
            that.lastEditClickLines.push({ editFrom, editTo });
            for (let j = editFrom; j < editTo; j++) {
              cm.removeLineClass(j, "background", `${that.classes.chunk}`);
              cm.removeLineClass(j, "background", `${that.classes.start}`);
              cm.removeLineClass(j, "background", `${that.classes.end}`);
              cm.addLineClass(
                j,
                "background",
                "CodeMirror-merge-lighlight-click-chunk"
  ...
  function registerUpdate(dv) {
    ...
    function update(mode) {
      ...
      makeConnections(dv);
      if (dv.needsScrollSync) syncScroll(dv, dv.needsScrollSync);
  ...
  // Liao: addHighlightClass(editor,lineNum,clesses,main,start,end)
  ...
  //   var hasLeft = origLeft, hasRight = origRight;
  ...
  var MergeView = (CodeMirror.MergeView = function (node, options) {
    ...
    var origLeft = options.origLeft,
      origRight = options.origRight,
      origMostRight = options.origMostRight,
      origRight04 = options.origRight04,
      origRight05 = options.origRight05;
    var hasLeft = origLeft,
      hasRight = origRight,
      hasMostRight = origMostRight,
      hasRight04 = origRight04,
      hasRight05 = origRight05;
    var panes =
      1 +
      (hasLeft ? 1 : 0) +
      (hasRight ? 1 : 0) +
    ...
    var wrap = [],
      ...
      mostRight = (this.mostRight = null),
      right04 = (this.right04 = null),
      right05 = (this.right05 = null);
    var self = this;
    var editPane = elt(
      "div",
      null,
    ...
    if (hasLeft) {
      ...
      wrap.push(leftPane);
      if (panes === 2) {
        leftPane.className += " CodeMirror-merge-pane-rightmost";
      }
    }
    ...
    if (hasMostRight) {
      mostRight = this.mostRight = new DiffView(this, "mostRight");
      wrap.push(buildGap(mostRight));
      var mostRightPane = elt(
        "div",
        null,
        "CodeMirror-merge-pane CodeMirror-merge-right"
      );
      wrap.push(mostRightPane);
      if (panes === 4) {
        mostRightPane.className += " CodeMirror-merge-pane-rightmost";
      }
    }

    if (hasRight04) {
      right04 = this.right04 = new DiffView(this, "right04");
      wrap.push(buildGap(right04));
      var right04Pane = elt(
        "div",
        null,
        "CodeMirror-merge-pane CodeMirror-merge-right"
      );
      wrap.push(right04Pane);
      if (panes === 5) {
        right04Pane.className += " CodeMirror-merge-pane-rightmost";
      }
    }

    if (hasRight05) {
      right05 = this.right05 = new DiffView(this, "right05");
      wrap.push(buildGap(right05));
      var right05Pane = elt(
        "div",
        null,
        "CodeMirror-merge-pane CodeMirror-merge-right"
      );
      wrap.push(right05Pane);
      if (panes === 6) {
        right05Pane.className += " CodeMirror-merge-pane-rightmost";
      }
    }
    ...
    if (left) left.init(leftPane, origLeft, options);
    if (right) right.init(rightPane, origRight, options);
    if (mostRight) mostRight.init(mostRightPane, origMostRight, options);
    if (right04) right04.init(right04Pane, origRight04, options);
    if (right05) right05.init(right05Pane, origRight05, options);

    if (left) left.registerEvents(right);
    if (right) right.registerEvents(left);
    if (mostRight) mostRight.registerEvents(mostRight);
    if (right04) right04.registerEvents(right04);
    if (right05) right05.registerEvents(right05);

    var onResize = function () {
      if (left) makeConnections(left);
      if (right) makeConnections(right);
      if (mostRight) makeConnections(mostRight);
      if (right04) makeConnections(right04);
      if (right05) makeConnections(right05);
    };
  ...
  function TrackAlignable(cm) {
    ...
    cm.on("markerAdded", function (_, marker) {
      ...
      if (found) self.set(found.line, F_MARKER);
    });
    cm.on("markerCleared", function (_, marker, _min, max) {
      if (max && marker.collapsed) self.check(max, F_MARKER, self.hasMarker);
    });
  ...
  function goNearbyDiff(cm, dir) {
    ...
    if (views)
      for (var i = 0; i < views.length; i++) {
        ...
        if (pos && (found == null || (dir < 0 ? pos > found : pos < found)))
          found = pos;
      }
    if (found) cm.setCursor(found, 0);
    ...
    for (
      ...
      for (var n = -v + r; n <= v - t; n += 2) {
        var l = g + n,
          m;
        m =
          n == -v || (n != v && j[l - 1] < j[l + 1]) ? j[l + 1] : j[l - 1] + 1;
      ...
      for (n = -v + p; n <= v - w; n += 2) {
        l = g + n;
        u =
          n == -v || (n != v && i[l - 1] < i[l + 1]) ? i[l + 1] : i[l - 1] + 1;
        for (
          m = u - n;
          u < d && m < e && a.charAt(d - u - 1) == b.charAt(e - m - 1);
        ...
        for (var h = 0, k, i = 0; i < a[f].diffs.length; i++) {
          ...
          1 === q[0]
            ...
            : -1 === q[0] &&
              (b =
                b.substring(0, j + k) +
                b.substring(j + this.diff_xIndex(g, h + q[1].length)));
          -1 !== q[0] && (h += q[1].length);
        }
      }
    }
